<script>
    ...
    onMount(() => {
        const handleSuccessfulPayment = (purchasedPlanId) => {
            ...
            if (purchasedPlanId && savedResult) {
                ...
                expressionTokens = []; // Очищаем историю старого уравнения
                waitingForNext = true; // Готовим к вводу нового числа поверх результата
    ...
    function calculateExpression(tokens) {
        ...
        // 1. Сначала считаем умножение и деление
        for (let i = 0; i < arr.length; i++) {
            if (arr[i] === '×' || arr[i] === '÷') {
                const a = parseFloat(arr[i - 1]);
                const b = parseFloat(arr[i + 1]);
                const res = arr[i] === '×' ? a * b : (b !== 0 ? a / b : 'Ошибка');
                arr.splice(i - 1, 3, String(res));
                i -= 2; // Сдвигаем индекс назад после склейки массива
            }
        }
    ...
    function handleNum(v) {
        if (waitingForNext) {
            ...
            currentVal = currentVal === '0' ? v : currentVal + v;
        }
        updateDisplay(currentVal);
    }

    function handleOp(op) {
        if (waitingForNext && expressionTokens.length > 0) {
            // Если пользователь передумал и нажал другой оператор (например, + а потом -)
            expressionTokens[expressionTokens.length - 1] = op;
        } else {
            // Закидываем число и знак в "копилку"
            expressionTokens.push(currentVal, op);
        }

        waitingForNext = true;
        historyText = expressionTokens.join(' ');
    }

    function handleEquals() {
        if (expressionTokens.length === 0) return; // Нечего считать

        if (!waitingForNext) {
            // Добавляем последнее введенное число в массив
            expressionTokens.push(currentVal);
        } else {
            // Если нажали = сразу после оператора (например "2 + ="), отбрасываем лишний плюс
        ...
        pendingResult = calculateExpression(expressionTokens);
...
<svelte:head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Unbounded:wght@300;400;600;700&family=JetBrains+Mono:wght@300;400;500&display=swap"
          rel="stylesheet"/>
</svelte:head>
...
<div class="wrapper">
    ...
    <div class="calc" bind:this={calcNode}>
        ...
        <div class="buttons">
            <button class="btn btn-clear" use:btnEffects
                    on:click={() => { currentVal='0'; historyText=''; expressionTokens=[]; waitingForNext=false; activePlan=null; updateDisplay('0'); }}>
                AC
            </button>
            <button class="btn btn-func" use:btnEffects
                    on:click={() => { currentVal = String(parseFloat(currentVal)*-1); updateDisplay(currentVal); }}>±
            </button>
            <button class="btn btn-func" use:btnEffects
                    on:click={() => { currentVal = String(parseFloat(currentVal)/100); updateDisplay(currentVal); }}>%
            </button>
            ...
            <button class="btn btn-num" use:btnEffects
                    on:click={() => { if(!currentVal.includes('.')) currentVal+='.'; updateDisplay(currentVal); }}>.
            </button>
        ...
        <div class="modal">
            ...
            <div class="modal-plans">
                {#each Object.values(PLANS) as plan}
                    <button class="plan plan-{plan.id}" class:selected={selectedPlan === plan.id}
            ...
            <div class="modal-footer">
                ...
                <button class="btn-pay" class:loading={isProcessing} disabled={!isAgreed || !selectedPlan}
                        on:click={processPay}>
...
<style>
    @keyframes -global-calPulse {
        0%, 100% {
            box-shadow: 0 0 0 0 rgba(252, 63, 29, 0);
        }
        50% {
            box-shadow: 0 0 0 8px rgba(252, 63, 29, 0.1);
        }
    }
    ...
    .display-main.flash {
        animation: numFlash 0.25s ease;
    }

    .display-main.result-show {
        color: var(--accent2);
    }
    ...
    .success-overlay.show .success-burst {
        animation: burstIn 1.8s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
    }
